...
if torch.backends.mps.is_available() and params.mps:
    print("WARNING: You are using MPS backend!")
# -----------------------------------------------
"""
In this block
...
def data_loader():
    ...
    # val
    val_dataset = dataset.ValDataset(root=args.valroot, transform=dataset.resizeNormalize((params.imgW, params.imgH)))
    assert val_dataset
...
if params.mps and torch.backends.mps.is_available():
...
def val(net, criterion):
    ...
    for i in range(max_iter):
        ...
        cpu_texts_decode = []
        for j in cpu_texts:
            cpu_texts_decode.append(j.decode('utf-8', 'strict'))
...
def train(net, criterion, optimizer, train_iter):
    ...
    preds_size = Variable(torch.LongTensor([preds.size(0)] * batch_size))
    cost = criterion(preds, text, preds_size, length) / batch_size
    #print(f"text: {text}, preds.size: {preds.size()}, cost: {cost}, type(cost): {type(cost)}, length: {length}, batch: {batch_size}")
    # crnn.zero_grad()
    cost.backward()
    optimizer.step()
    return cost 
